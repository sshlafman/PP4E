...
class Filter(MediaTool):
    def open(self, mediafile, **ignored):
        ...
        player.write(media.read())                   # send it its stdin
...
#####################################################################################
# media- and platform-specific policies: change me, or pass me in
#####################################################################################
...
apptools = {
            'win32': Winstart()   # doc, xls, etc: sue it on your own risk
            }
...
def playknownfile(filename, playertable={}, **options):
    ...
    if sys.platform in playertable:
        ...
        trywebbrowser(filename, **options)                   # general schemes
        
def playfile(filename, mimetable=mimetable, **options):
    """
    play media file of any type: uses imetypes to guess meia
    ...
    if maintype == 'text' and subtype == 'html':             # special case
