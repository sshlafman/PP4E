...
some of this module may assume "python" is on your system path (see launcheer.py);
...
/ to \ for Windows tools where required; fix is inherited b y PyEdit and others;
...
class LaunchMode:
    """
    on call to instance, announce lable and run command;
    subclasses format command lines as required in run();
    command should begin with name of Python script
    file to run, and not with "python" of its full path;
    """
    ...
    def announce(self, text):     # subclasses may redefine announce()
        print(text)               # instead of if/elif logic
...
class Fork(LaunchMode):
    """
    run command in explicitely created new process
...
class StartArgs(LaunchMode):
    ...
    def run(self, cmdline):
        assert sys.platform[:3] == 'win'
        os.system('start ' + cmdline)        # may create popup window
        
class Spawn(LaunchMode):
    """
    run python in new process independent of caller
    for Windows or Unix; use P_NOWAIT for dox box;
    forward shashes are okay here
    """
    def run(self, cmdline):
...
#
# pick a "best" launcher for this patform
